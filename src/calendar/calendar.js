...
class CalendarDash extends Component {
  ...
  openModal() {
    ...
    this.setState({ 
      ...
      project1: data2.days[day].tasks[0].project,
      project2: data2.days[day].tasks[1].project,
  ...
  componentDidMount() {
    ...
    setTimeout(() => {
      ...
      read(localStorage.getItem("currentEmployee")).then(data1 => {
        let currMonth = data1.currentMonth;
        getOne(currMonth).then(data2 => {
          let imeIPrezime = data1.firstName + " " + data1.lastName;
          console.log("CDM");
          console.log(data2.days[day].tasks[0].projectId);
          this.setState({
            currentMonthId: currMonth,
            employeesMonth: data2,
            employeesName: imeIPrezime,
            comments: data2.days[day].comments,
            type : data2.days[day].type,
            description1: data2.days[day].tasks[0].description,
            description2: data2.days[day].tasks[1].description,
  ...
  submitChanges = () => {
    let cm = this.state.employeesMonth.days;
    let day = this.state.selectedDate.getDate();
    day--;

    cm[day].type = this.state.type || ""; 
    cm[day].hours = this.state.hours || ""; 
    cm[day].tasks[0].projectId = this.state.project1 || "";
    cm[day].tasks[1].projectId = this.state.project2 || "";
    cm[day].tasks[0].description = this.state.description1 || "";
    cm[day].tasks[1].description = this.state.description2 || "";
    ...
    update(this.state.employeesMonth._id, newEmpMonth).then(data => {
      if (data.error) {
        console.log(data.error);
      }
    });
